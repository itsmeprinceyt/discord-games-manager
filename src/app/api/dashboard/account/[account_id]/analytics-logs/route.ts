...
import getSingleAnalyticsLogsRedisKey from "../../../../../../utils/Redis/getSingleAnalyticsLogsRedisKey";
import { SINGLE_USER_ANALYTICS_LOGS } from "../../../../../../utils/Redis/redisTTL";
...
export async function GET(
  ...
  try {
    ...
    const cacheKey = `${getSingleAnalyticsLogsRedisKey()}:${
      session.user.id
    }:${accountId}`;
    ...
    const [crosstrades] = await pool.execute<any[]>(
      `SELECT 
        ...
        note
       FROM crosstrades 
       WHERE user_id = ? AND bot_account_id = ?
    ...
    if (!crosstrades || crosstrades.length === 0) {
      ...
      await redis.set(cacheKey, emptyData, { ex: SINGLE_USER_ANALYTICS_LOGS });
    ...
    await redis.set(cacheKey, responseData, { ex: SINGLE_USER_ANALYTICS_LOGS });
